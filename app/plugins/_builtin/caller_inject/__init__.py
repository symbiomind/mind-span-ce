...
Config (all optional — falls back to identity context values):
...
The config loader auto-prepends caller_inject with {name, trust} when
...
def hook(hook_point: str, ctx: dict, config: dict) -> dict | None:
    name = config.get("name") or ctx.get("identity", {}).get("name")
    trust = config.get("trust") or ctx.get("identity", {}).get("trust")
